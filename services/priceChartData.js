...
const cache = {
  ...
  timestamp: 0,
  ttl: 60000,         // Cache TTL in milliseconds (20 minutes)
  isExpired() {
...
function createAsciiPriceChart(priceData, width = 50, height = 10) {
  try {
    console.log(`Creating ASCII chart with ${priceData.length} data points`);
    ...
    console.log("ASCII chart created successfully");
    return result;
  } catch (err) {
    console.error("Error creating ASCII chart:", err);
...
async function fetchPriceData() {
  ...
  try {
    ...
    console.log(`Successfully fetched ${data.length} price data points`);
    return data;
  } catch (err) {
...
async function renderPriceChart(priceData) {
  try {
    if (!priceData || priceData.length === 0) {
      console.error("No price data available to render");
    ...
    console.log("Successfully rendered price chart");
    return response;
  } catch (err) {
    console.error("Error rendering price chart:", err);
    return [
      "Error rendering price chart",
      `Error: ${err.message}`,
...
async function updateCache() {
  ...
  try {
    // Create fallback data in case fetch fails
    const fallbackData = [];
    // Generate some mock price data if needed as fallback
    const now = Date.now();
    for (let i = 0; i < 24; i++) {
      fallbackData.push({
        timestamp: now - (23 - i) * 3600000,
        price: 60 + Math.random() * 10,
        volume_24h: 1000000000 + Math.random() * 200000000
      });
    }
    ...
    let freshData;
    try {
      freshData = await fetchPriceData();
      console.log(`Successfully fetched ${freshData.length} data points`);
    } catch (fetchErr) {
      console.error("Error fetching price data, using fallback:", fetchErr.message);
      // Use fallback data if fetch fails
      freshData = fallbackData;
    }

    // Pre-render the chart
    console.log("Pre-rendering price chart...");
    ...
    // Create emergency fallback if rendering also fails
    cache.renderedChart = [
      "SOL Price Data Service",
      "------------------------",
      "Error: Unable to refresh price data",
      "Server is experiencing technical difficulties",
      "Please try again later",
      `Last error: ${err.message}`,
      `Attempted refresh at: ${new Date().toLocaleString()}`
    ];
...
// Function to get the pre-rendered price chart (returns immediately)
function getPriceChartLines() {
  // If we have any cached data (even if expired), return it immediately
  if (cache.renderedChart) {
    // If expired, trigger background refresh but don't wait for it
    if (cache.isExpired()) {
      console.log("Cache is expired but returning stale data while refreshing");
      // Use setTimeout to avoid blocking this function call
      setTimeout(() => {
  ...
  // This should only happen on first server start before init completes
...
function initializeCache() {
  ...
  // Start the first update process
  updateCache()
    .then(success => {
      if (success) {
        console.log("Initial cache load successful");
      } else {
        console.error("Initial cache load failed");
...
export default {
  ...
  getCacheStatus() {
    return {
      hasData: !!cache.data,
      hasRenderedChart: !!cache.renderedChart,
